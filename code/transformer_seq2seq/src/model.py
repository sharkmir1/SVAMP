...
from transformers import AdamW, BartForConditionalGeneration, AutoTokenizer
...
def train_model(model, train_dataloader, val_dataloader, voc1, voc2, device, config, logger, epoch_offset=0,
                ...
                writer=None, tokenizer=AutoTokenizer.from_pretrained("facebook/bart-large")):
    """
        Args:
            ...
            max_val_bleu (float): maximum validation bleu score
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for data in train_dataloader:
            ...
            # Make string again from indices (for BART)
            questions = idx_to_sents(voc1, sent1_var)
            questions = [" ".join(list(filter(lambda x: x != "</s>", question)))
                         for question in questions]
            question_encoding = tokenizer(questions, return_tensors='pt', padding=True)

            equations = idx_to_sents(voc2, sent2_var)
            equations = [" ".join(list(filter(lambda x: x != "<s>" and x != "</s>", equation)))
                         for equation in equations]
            equation_encoding = tokenizer(equations, return_tensors='pt', padding=True)

            generation_labels = equation_encoding.input_ids.masked_fill(
                equation_encoding.input_ids == model.config.pad_token_id,
                -100
            )
            ...
            if config.show_train_acc:
                ...
                output = model.generate(
                    input_ids=question_encoding.input_ids.to(device),
                    attention_mask=question_encoding.attention_mask.to(device),
                    max_length=question_encoding.size(1) + 20,
                    num_beams=5,
                    early_stopping=True,
                )
